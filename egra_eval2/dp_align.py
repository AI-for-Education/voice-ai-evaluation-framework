...
class DPEntry:
    """Alignment type ("d", "i", "s", or "c") and an integer score."""

    def __init__(self, align="c", score=0):
        self.align = align
        self.score = score
...
def dp_align(
    ...
    if output_align:
        alignment = []
    while i > 0 or j > 0:
        if dp_matrix[i, j].align == "c":
            if output_align:
                alignment.append((test_list[i - 1], ref_list[j - 1], "c"))
            i -= 1
            j -= 1
            dp_errors.n_match += 1
        elif dp_matrix[i, j].align == "s":
            if output_align:
                alignment.append((test_list[i - 1], ref_list[j - 1], "s"))
            i -= 1
            j -= 1
            dp_errors.n_sub += 1
        elif dp_matrix[i, j].align == "d":
            if output_align:
                alignment.append(("-", ref_list[j - 1], "d"))
            j -= 1
            dp_errors.n_del += 1
        elif dp_matrix[i, j].align == "i":
            if output_align:
                alignment.append((test_list[i - 1], "-", "i"))
            i -= 1
            dp_errors.n_ins += 1

    # Return the alignment results
    if output_align:
        return (dp_errors, alignment[::-1])
    return dp_errors
